...
/* ── Background particle definitions ─────────────────────────── */
...
export default function SignInPage() {
  ...
  useEffect(() => {
    let raf: number;
    let tiltX = 0, tiltY = 0, bob = 0;
    let robotX = window.innerWidth / 2, robotY = window.innerHeight / 2;
    ...
    const tick = () => {
      ...
      /* 2. Card 3D tilt + idle bob */
      if (cardRef.current) {
        const r = cardRef.current.getBoundingClientRect();
        const over = mx > r.left && mx < r.right && my > r.top && my < r.bottom;
        const ttX = over ? ((my - r.top)  / r.height - 0.5) * 12 : 0;
        const ttY = over ? ((mx - r.left) / r.width  - 0.5) * -12 : 0;
        cardRef.current.style.boxShadow = over
          ? '0 32px 80px rgba(18,18,16,0.15), 0 8px 24px rgba(18,18,16,0.08)'
          : '0 10px 36px rgba(18,18,16,0.09), 0 2px 8px rgba(18,18,16,0.04)';
        tiltX += (ttX - tiltX) * 0.15;
        tiltY += (ttY - tiltY) * 0.15;
        const tBob = over ? 0 : Math.sin(Date.now() / 600) * -5;
        bob += (tBob - bob) * 0.1;
        cardRef.current.style.transform = `translateY(${bob}px) rotateX(${tiltX}deg) rotateY(${tiltY}deg) translateZ(${over ? 15 : 0}px)`;
      }
      ...
      if (robotRef.current) {
        ...
        const rX = ((robotY / H) - 0.5) * -45;
        const rY = ((robotX / W) - 0.5) * 45;
        robotRef.current.style.transform = `translate3d(${robotX - 200}px,${robotY - 200}px,-100px) rotateX(${rX}deg) rotateY(${rY}deg)`;
        if (robotEye1.current && robotEye2.current) {
          const ex = ((mx / W) - 0.5) * 40;
          const ey = ((my / H) - 0.5) * 40;
          robotEye1.current.style.transform = `translate3d(${ex}px,${ey}px,0)`;
          robotEye2.current.style.transform = `translate3d(${ex}px,${ey}px,0)`;
        }
      }
  ...
  return (
    <>
      ...
      <div style={{ position: 'fixed', inset: 0, zIndex: 0, perspective: '1200px', pointerEvents: 'none', overflow: 'hidden' }}>
        <div ref={robotRef} style={{ width: 400, height: 400, position: 'absolute', top: 0, left: 0, transformStyle: 'preserve-3d', willChange: 'transform', opacity: 0.15 }}>
          ...
          <div style={{ position: 'absolute', inset: 0, background: '#EDE5D4', border: '16px solid #121210', borderRadius: 80, boxShadow: 'inset 0 -30px 0 rgba(0,0,0,0.06)', display: 'flex', alignItems: 'center', justifyContent: 'center', transformStyle: 'preserve-3d' }}>
            <div style={{ width: '75%', height: '55%', background: '#121210', borderRadius: 40, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 60, position: 'relative', overflow: 'hidden', transform: 'translateZ(-10px)' }}>
      ...
      {P.map((p, i) => (
        <div key={i} ref={el => { pels.current[i] = el; }}
          style={{ position: 'fixed', left: 0, top: 0, width: p.w, height: p.h, background: p.bg, borderRadius: p.br, opacity: 0.58, pointerEvents: 'none', willChange: 'transform', zIndex: 1 }}
        />
      ))}

      <div style={{ minHeight: '100vh', background: 'radial-gradient(ellipse at 45% 50%, #F9F3E4 0%, #ECE6D8 60%, #DDD5C5 100%)', display: 'flex', alignItems: 'center', justifyContent: 'center', overflow: 'hidden', position: 'relative', fontFamily: 'Inter, system-ui, sans-serif' }}>
        <div style={{ position: 'absolute', top: -180, left: -180, width: 640, height: 640, borderRadius: '50%', border: '1.5px solid rgba(243,215,106,0.35)', animation: 'ringRotate 75s linear infinite', pointerEvents: 'none', zIndex: 0 }} />
        <div style={{ position: 'absolute', bottom: -150, right: -150, width: 480, height: 480, borderRadius: '50%', border: '1.5px solid rgba(168,216,185,0.3)', animation: 'ringRotate 60s linear infinite reverse', pointerEvents: 'none', zIndex: 0 }} />
        ...
        <div style={{ perspective: '900px', zIndex: 10, position: 'relative' }}>
          <div ref={cardRef} style={{ width: 420, background: '#FEFBEF', border: '1.5px solid #D8CFBE', borderRadius: 22, padding: '40px 36px 36px', willChange: 'transform', position: 'relative' }}>
            ...
            <div style={{ textAlign: 'center', marginBottom: 20 }}>
              ...
              <div style={{ fontSize: 12, color: '#8B867C', marginTop: 4 }}>Welcome back — let&apos;s get to work</div>
            </div>
            ...
            <p style={{ margin: '16px 0 0', textAlign: 'center', fontSize: 13, color: '#8B867C' }}>
              Don&apos;t have an account?{' '}
              <a href="/sign-up" style={{ color: '#121210', fontWeight: 700, textDecoration: 'underline', textUnderlineOffset: 3 }}>Sign up</a>
            </p>
          </div>
        </div>
      </div>
